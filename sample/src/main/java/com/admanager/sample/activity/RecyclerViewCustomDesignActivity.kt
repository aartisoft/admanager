 ...
 */
class RecyclerViewCustomDesignActivity : AppCompatActivity() {
    var trackAdapter: TrackAdapterCustomDesign? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_native_list)
        trackAdapter = TrackAdapterCustomDesign(this)

        recyclerView.apply {
            val layout = LinearLayoutManager(this@RecyclerViewCustomDesignActivity)
            layoutManager = layout
            // set adapter
            adapter = trackAdapter
            // divider
            addItemDecoration(
                DividerItemDecoration(this@RecyclerViewCustomDesignActivity, layout.orientation)
            )
        }
