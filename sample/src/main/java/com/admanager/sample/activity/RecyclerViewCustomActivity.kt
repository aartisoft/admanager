...
import com.admanager.sample.adapter.TrackAdapterCustom
...
class RecyclerViewCustomActivity : AppCompatActivity() {
    var trackAdapter: TrackAdapterCustom? = null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_native_list)
        trackAdapter = TrackAdapterCustom(this)

        recyclerView.apply {
            val layout = LinearLayoutManager(this@RecyclerViewCustomActivity)
            layoutManager = layout
            // set adapter
            adapter = trackAdapter
            // divider
            addItemDecoration(
                DividerItemDecoration(this@RecyclerViewCustomActivity, layout.orientation)
            )
        }
